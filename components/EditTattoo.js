...
export default function TattooEditor() {
  ...
  async function handleSubmit(event) {
    ...
    .then(async (response) => {
      ...
      setOpenAIResponse(data); // Assuming the response contains the prompt
    });

    // Call DALL-E API to generate image based on prompt
    if (openAIResponse) {
      await fetch("/api/dallEImageNew", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      .then(async (response) => {
        const data = await response.json();
        setGeneratedImage(data); // Assuming the response contains the generated image data
      });
    }
  }
      ...
      <div className="bg-slate-800 w-full max-w-2xl rounded-lg shadow-md p-8">
        ...
        {generatedImage && (
          <div className="border-t border-gray-300 pt-4">
            ...
            <img src={generatedImage} className="w-full object-contain max-h-72" alt="Generated" />
          </div>
        )}
      </div>
    </div>

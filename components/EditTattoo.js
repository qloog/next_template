...
export default function EditTattoo() {
  ...
  async function handleSubmit(event) {
    ...
    if (image === "" || modificationRequest.trim() === "") {
      alert("Please upload an image and specify your modification request before submitting.");
      return;
    }
    ...
    fetch("/api/editUserImage", {
      ...
      body: JSON.stringify({ image: image, modificationRequest: modificationRequest }),
    })
    .then(async (response) => {
        if (!response.body) {
